...
export const config = {
  ...
  deleteJsSeedEnabled: toBool(process.env.DELETE_JS_SEED_ENABLED, true),
  deleteJsSeedMaxChars: Math.max(500, toNumber(process.env.DELETE_JS_SEED_MAX_CHARS, 7000)),
  deleteJsSeedCacheTtlMs: Math.max(5_000, toNumber(process.env.DELETE_JS_SEED_CACHE_TTL_MS, 5 * 60 * 1000)),
  scopedAllowlistEnabled: toBool(process.env.SCOPED_ALLOWLIST_ENABLED, true),
  scopedAllowlistMaxTables: Math.max(3, toNumber(process.env.SCOPED_ALLOWLIST_MAX_TABLES, 10)),
  wmsTrainingContextEnabled: toBool(process.env.WMS_TRAINING_CONTEXT_ENABLED, true),
  wmsTrainingContextCacheTtlMs: Math.max(10_000, toNumber(process.env.WMS_TRAINING_CONTEXT_CACHE_TTL_MS, 5 * 60 * 1000)),
  wmsTrainingContextMaxPromptChars: Math.max(800, toNumber(process.env.WMS_TRAINING_CONTEXT_MAX_PROMPT_CHARS, 4500)),
  wmsTrainingContextPromptMaxTables: Math.max(1, toNumber(process.env.WMS_TRAINING_CONTEXT_PROMPT_MAX_TABLES, 6)),
  wmsTrainingContextRoutingMaxTables: Math.max(1, toNumber(process.env.WMS_TRAINING_CONTEXT_ROUTING_MAX_TABLES, 6)),
